...
import { MapPin, AlertCircle, Activity, PlugZap } from "lucide-react";
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center">
                ...
                <div className="text-xs text-muted-foreground ml-2">
                  ({Math.round((errorCount / devices.length) * 100)}%)
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-0 h-full flex items-center justify-center bg-muted/40 relative overflow-hidden">
              ...
              <div className="text-center p-8">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  The map will show device locations from your account
                </p>
                {hoveredDevice && (
                  <div className="mt-8 border rounded-lg p-4 bg-card max-w-xs mx-auto text-left">
                    ...
                    <div className="mt-2 text-sm">
                      <p><span className="text-muted-foreground">Status:</span> {hoveredDevice.status}</p>
                      <p><span className="text-muted-foreground">Location:</span> {hoveredDevice.location.city}, {hoveredDevice.location.country}</p>
                      <p><span className="text-muted-foreground">Coordinates:</span> {hoveredDevice.location.latitude.toFixed(4)}, {hoveredDevice.location.longitude.toFixed(4)}</p>
                    </div>
                  </div>
                )}
              </div>
